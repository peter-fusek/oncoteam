...
<template>
  <div class="space-y-6">
      ...
      >
        <div class="px-4 py-3 border-b border-gray-100 flex items-center justify-between">
          <div class="flex items-center gap-2">
            <span class="font-medium text-gray-900 text-sm">{{ session.title }}</span>
            <UBadge
              size="xs"
              variant="subtle"
        ...
        <div class="px-4 py-3">
          <p class="text-sm text-gray-500 whitespace-pre-line">{{ session.content }}</p>
        </div>
        <div v-if="session.tags?.length" class="px-4 py-2 border-t border-gray-100 flex gap-1 flex-wrap">
    ...
    <div v-else-if="!sessions?.error && !sessionsError" class="text-gray-500 text-center py-16 text-sm">
