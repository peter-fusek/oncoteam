      ...
      <div v-for="(item, ii) in section.items" :key="item" class="flex items-center gap-2">
        ...
        <template v-if="readonly && labValues">
          <template v-if="getLabEval(si, ii)">
            <UIcon
              ...
              :class="getLabEval(si, ii)!.pass ? 'text-green-500' : 'text-red-600'"
              class="w-4 h-4 shrink-0"
            />
            <span class="text-sm" :class="getLabEval(si, ii)!.pass ? 'text-gray-700' : 'text-red-600'">
              {{ item }}
          ...
          <template v-else>
            <UIcon name="i-lucide-minus" class="text-gray-500 w-4 h-4 shrink-0" />
        ...
        <template v-else>
          <input
            ...
            type="checkbox"
            class="rounded border-gray-300 bg-gray-100 text-teal-600 focus:ring-teal-500/30 w-3.5 h-3.5"
          />
          <span class="text-sm text-gray-700 flex-1" :class="{ 'line-through text-gray-500': checked[item] }">{{ item }}</span>
          ...
          <template v-if="getLastLabInfo(si, ii)">
            ...
            <span v-if="getLastLabInfo(si, ii)!.sample_date" class="text-xs text-gray-500 shrink-0">
