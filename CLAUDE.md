...
- `src/oncoteam/server.py` — MCP server, 18 tools + 14 dashboard API routes (2 POST, 1 parameterized), system instructions with biomarker rules + QA protocol
- `src/oncoteam/dashboard_api.py` — Dashboard JSON API: /api/{status,activity,stats,timeline,patient,research,sessions,autonomous,protocol,briefings,toxicity,labs,diagnostics,detail/{type}/{id}}
